...
END:VCARD`
    ...
    const renderTemplate = () => {
        ...
        if (templateId === 'neon_modern_pop') {
            return <NeonModernPopTemplate {...props} tone={tone} toneStyle={toneStyle} translateText={translateText} />;
        }
        if (templateId === 'clean_slate_min') {
            return <CleanSlateTemplate {...props} tone={tone} toneStyle={toneStyle} translateText={translateText} />;
        }
        if (templateId === 'creative_prem') {
            return <CreativePremiumTemplate {...props} tone={tone} toneStyle={toneStyle} translateText={translateText} />;
        }
        if (templateId === 'cyber_link_elite') {
            return <CyberEliteTemplate {...props} tone={tone} toneStyle={toneStyle} translateText={translateText} />;
        }
...
function NeonModernPopTemplate({ profile, handleShare, handleCVView, reviews, setIsReviewModalOpen, setIsAppointmentOpen, isAppointmentOpen, t, trackEvent, toneStyle, setIsLeadModalOpen, isLeadModalOpen, setIsQrOpen, setLang, lang, setIsWalletModalOpen, isWalletModalOpen, aiConfig, setIsAIChatOpen, translateText, cvViewUrl, setIsCVModalOpen, isCVModalOpen }: any) {
    const theme = {
        bg: "bg-[#020617]",
        card: "bg-white/5",
        accent: "#ff007f", // Neon Pink
        secondary: "#00f2ff", // Neon Blue
        text: "text-white",
    ...
    return (
        <div className={cn("min-h-screen relative overflow-x-hidden selection:bg-pink-500/30", theme.bg, toneStyle.font)}>
            ...
            {/* Navigation */}
            <header className="sticky top-0 z-[100] px-6 py-4 flex justify-between items-center backdrop-blur-2xl bg-black/40 border-b border-white/5">
                <div className="flex gap-2">
                    <button onClick={() => setIsQrOpen(true)} className="w-10 h-10 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-white/70 hover:bg-white/10 transition-all hover:scale-105 active:scale-95 shadow-xl">
                        <QrCode size={18} />
                    </button>
                    {aiConfig?.isEnabled && (
                        <button onClick={() => setIsAIChatOpen(true)} className="w-10 h-10 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-white/70 hover:bg-white/10 transition-all hover:scale-105 active:scale-95 shadow-xl">
                            <Bot size={18} />
                        </button>
                    )}
                </div>
                <div className="flex gap-3 items-center">
                    <button onClick={() => setLang(lang === 'tr' ? 'en' : 'tr')} className="px-3 py-1.5 rounded-xl bg-white/5 border border-white/10 text-white/50 font-black text-[10px] uppercase tracking-widest hover:text-white transition-all shadow-xl">
                        {lang.toUpperCase()}
                    </button>
                    <button onClick={() => setIsWalletModalOpen(true)} className="w-10 h-10 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-white/70 hover:bg-white/10 transition-all hover:scale-105 active:scale-95 shadow-xl">
                        <UserPlus size={18} />
                    </button>
                </div>
            </header>

            <main className="relative z-10 w-full max-w-lg mx-auto pb-48">
                {/* Curved Header Section (Elite Style but Neon) */}
                <div className="relative h-72 sm:h-80 w-full overflow-hidden">
                    <img 
                        src={profile.profileBgImage || "/images/patterns/cyber_abstract.jpg"} 
                        className="w-full h-full object-cover grayscale-[0.3] brightness-50"
                        alt="bg"
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent to-black/20" />
                    <svg className="absolute bottom-[-1px] w-full h-24 text-[#020617] fill-current" preserveAspectRatio="none" viewBox="0 0 1440 320">
                        <path d="M0,160L48,176C96,192,192,224,288,224C384,224,480,192,576,165.3C672,139,768,117,864,128C960,139,1056,181,1152,192C1248,203,1344,181,1392,170.7L1440,160L1440,320L1392,320C1344,320,1248,320,1152,320C1056,320,960,320,864,320C768,320,672,320,576,320C480,320,384,320,288,320C192,320,96,320,48,320L0,320Z"></path>
                    </svg>
                    
                    {/* Floating Skill Tags (Pop Style) */}
                    <div className="absolute top-10 right-6 flex flex-col gap-2">
                        <motion.div initial={{ x: 50, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.5 }} className="bg-white/10 backdrop-blur-md px-4 py-1.5 rounded-full border border-white/20 shadow-2xl">
                            <span className="text-[10px] font-black text-white/70 uppercase tracking-widest">{profile.occupation || "PROFESSIONAL"}</span>
                        </motion.div>
                    </div>
                </div>

                {/* Identity Area */}
                <div className="px-6 -mt-20 flex flex-col items-center text-center space-y-6">
                    <div className="relative group">
                        <div className="absolute -inset-1 rounded-full bg-gradient-to-tr from-pink-500 to-cyan-500 blur opacity-75 group-hover:opacity-100 transition duration-1000 group-hover:duration-200 animate-tilt"></div>
                        <div className="relative w-36 h-36 rounded-full p-1 bg-black">
                            <img 
                    ...
                    <div className="space-y-3">
                        <h1 className="text-4xl font-black italic tracking-tighter text-transparent bg-clip-text bg-gradient-to-tr from-white to-white/60 uppercase leading-none drop-shadow-2xl">
                            {profile?.user?.name || ""}
                        </h1>
                        <div className="flex items-center justify-center gap-2">
                             <div className="h-[1px] w-8 bg-pink-500" />
                             <p className="text-[12px] font-black uppercase tracking-[0.4em] text-pink-500 drop-shadow-[0_0_8px_rgba(236,72,153,0.5)]">
                                {translateText(profile.occupation)}
                             </p>
                             <div className="h-[1px] w-8 bg-pink-500" />
                        </div>
                        {profile.slogan && (
                            <p className="text-[11px] font-bold text-white/40 italic tracking-widest uppercase px-10">
                                “{translateText(profile.slogan)}”
                            </p>
                        )}
                    </div>
                ...
                {/* Quick Action Buttons (Glassmorphism + Neon) */}
                <div className="px-6 mt-12 space-y-4">
                    {socialLinks.filter((l: any) => l.platform === 'phone' || l.platform === 'whatsapp' || l.platform === 'website').map((link: any, i: number) => (
                        <motion.a
                            key={i}
                            href={link.platform === 'phone' ? `tel:${link.url}` : link.platform === 'whatsapp' ? `https://wa.me/${link.url.replace(/\D/g, '')}` : link.url}
                            whileHover={{ scale: 1.02, x: 5 }}
                            whileTap={{ scale: 0.98 }}
                            className="w-full h-20 rounded-[2rem] bg-white/[0.03] border border-white/5 backdrop-blur-3xl flex items-center gap-6 px-10 hover:bg-white/[0.08] transition-all group relative overflow-hidden shadow-2xl"
                        >
                            <div className="absolute inset-0 bg-gradient-to-r from-transparent via-pink-500/5 to-transparent -translate-x-full group-hover:translate-x-full duration-1000" />
                            <div className="w-12 h-12 rounded-2xl flex items-center justify-center bg-pink-500/10 text-pink-500 shadow-[0_0_20px_rgba(236,72,153,0.2)] group-hover:scale-110 transition-transform">
                                {link.platform === 'phone' && <Phone size={20} />}
                                {link.platform === 'whatsapp' && <MessageCircle size={20} />}
                                {link.platform === 'website' && <Globe size={20} />}
                            </div>
                            <div className="flex flex-col items-start translate-y-0.5">
                                <span className="text-[8px] font-black text-pink-500/40 uppercase tracking-[0.4em] mb-0.5">{link.platform}</span>
                                <span className="text-[13px] font-black text-white/80 uppercase tracking-widest leading-none">{link.platform === 'phone' ? t.phoneCallsBtn || "HEMEN ARA" : link.platform === 'whatsapp' ? "WHATSAPP'DAN YAZ" : "WEB SİTEYİ GÖR"}</span>
                            </div>
                            <ArrowRight size={18} className="ml-auto text-white/20 group-hover:text-pink-500 transform group-hover:translate-x-2 transition-all" />
                        </motion.a>
                    ))}
                    ...
                    <motion.button
                        ...
                        whileHover={{ scale: 1.02, x: 5 }}
                        whileTap={{ scale: 0.98 }}
                        className="w-full h-20 rounded-[2rem] bg-gradient-to-tr from-pink-600 to-purple-600 flex items-center gap-6 px-10 shadow-[0_20px_40px_-10px_rgba(219,39,119,0.4)] group active:translate-y-1"
                    >
                        <div className="w-12 h-12 rounded-2xl bg-white/20 flex items-center justify-center text-white backdrop-blur-md">
                            <MessageSquare size={20} />
                        </div>
                        <span className="text-[13px] font-black text-white uppercase tracking-[0.2em]">{t.contactMeTitle || "MESAJ GÖNDER"}</span>
                        <Zap size={18} className="ml-auto text-white/60 animate-pulse" />
                    </motion.button>
                </div>
                ...
                <div className="px-6 mt-16 space-y-16">
                    ...
                    {profile.services?.length > 0 && (
                        <div className="space-y-8">
                            <div className="flex items-center gap-4">
                                <div className="h-[2px] w-12 bg-pink-500/30" />
                                <h2 className="text-[10px] font-black uppercase tracking-[0.5em] text-pink-500 drop-shadow-[0_0_8px_rgba(236,72,153,0.3)]">{t.servicesTitle || "HİZMETLER"}</h2>
                            </div>
                            <div className="grid grid-cols-1 gap-4">
                                {profile.services.map((item: any, i: number) => (
                                    <div key={i} className="group p-8 rounded-[2.5rem] bg-white/[0.02] border border-white/5 hover:bg-white/[0.05] transition-all relative overflow-hidden backdrop-blur-3xl">
                                        <div className="absolute top-0 right-0 w-32 h-32 bg-pink-500/5 blur-3xl rounded-full" />
                                        <h4 className="text-sm font-black text-white mb-2 uppercase tracking-widest">{translateText(item.title)}</h4>
                                        <p className="text-[11px] text-white/40 leading-relaxed font-medium">{translateText(item.description)}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
                    ...
                    {/* Blog/Products with Shimmer Effect */}
                    {profile.products?.length > 0 && (
                        <div className="space-y-8">
                            <div className="flex items-center gap-4">
                                <div className="h-[1px] flex-1 bg-white/5" />
                                <h2 className="text-[10px] font-black uppercase tracking-[0.5em] text-cyan-400 drop-shadow-[0_0_8px_rgba(34,211,238,0.3)]">GALERİ</h2>
                                <div className="h-[1px] flex-1 bg-white/5" />
                            </div>
                            <div className="grid grid-cols-1 gap-6">
                                {profile.products.filter((p: any) => p.image).map((item: any, i: number) => (
                                    <motion.div 
                                        key={i} 
                                        whileHover={{ y: -5 }}
                                        onClick={() => item.link && window.open(item.link, '_blank')}
                                        className="rounded-[3rem] overflow-hidden bg-white/[0.03] border border-white/5 group relative shadow-2xl cursor-pointer"
                                    >
                                        <div className="aspect-video relative overflow-hidden">
                                            <img src={item.image} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110 grayscale-[0.5] group-hover:grayscale-0" alt="" />
                                            <div className="absolute inset-0 bg-gradient-to-t from-[#020617] via-transparent to-transparent opacity-90" />
                                            <div className="absolute bottom-6 left-8 right-8 flex justify-between items-end">
                                                <div className="space-y-1">
                                                    <h3 className="text-sm font-black text-white uppercase tracking-widest">{translateText(item.name)}</h3>
                                                    <p className="text-[10px] text-white/40 font-bold uppercase tracking-widest">{item.price > 0 ? `${item.price} ₺` : "DETAYLAR"}</p>
                                                </div>
                                                <div className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center text-white backdrop-blur-md opacity-0 group-hover:opacity-100 transform translate-y-4 group-hover:translate-y-0 transition-all">
                ...
                <div className="py-20 flex flex-col items-center gap-8">
                     ...
                     <Link href="/" className="flex items-center gap-2 group">
                        <div className="w-6 h-6 bg-pink-600 rounded-full flex items-center justify-center shadow-[0_0_15px_rgba(219,39,119,0.5)] group-hover:scale-110 transition-transform">
                            <Layout className="text-white w-3 h-3" />
                        </div>
                        <span className="text-sm font-black tracking-tighter text-white/30 group-hover:text-white/60 transition-colors">Kardly<span className="text-pink-600">.site</span></span>
                    </Link>
                </div>
            </main>

            {/* Bottom Bar (Elite Floating Style) */}
            <div className="fixed bottom-10 left-1/2 -translate-x-1/2 w-full max-w-[420px] px-6 z-[150]">
                <div className="bg-white/[0.03] backdrop-blur-[40px] border border-white/10 rounded-[3rem] p-3 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)] flex items-center justify-between gap-3">
                    <button onClick={handleShare} className="w-14 h-14 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-white/40 hover:text-white transition-all shadow-xl hover:bg-white/10 group">
                        <Share2 size={20} className="group-hover:rotate-12 transition-transform" />
                    </button>

                    <button
                        onClick={handleCVView}
                        className="flex-1 h-14 rounded-full bg-pink-600 text-white text-[11px] font-black uppercase tracking-[0.3em] shadow-[0_15px_30px_-5px_rgba(219,39,119,0.5)] active:translate-y-1 transition-all relative overflow-hidden group"
                    >
                        <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent -translate-x-full group-hover:translate-x-full duration-1000" />
                        <span className="relative z-10">{profile.isCatalog ? (t.viewCatalog || "KATALOG") : (t.viewCV || "CV GÖRÜNTÜLE")}</span>
                    </button>

                    <button onClick={() => setIsAppointmentOpen(true)} className="w-14 h-14 rounded-full bg-white text-black flex items-center justify-center shadow-xl hover:scale-105 active:scale-95 transition-all">
                        <Calendar size={20} />
                    </button>
                    
                    {aiConfig?.isEnabled && (
                        <button onClick={() => setIsAIChatOpen(true)} className="w-14 h-14 rounded-full bg-black border-2 border-pink-600 flex items-center justify-center text-pink-500 shadow-xl hover:scale-105 active:scale-95 transition-all group">
...
function CleanSlateTemplate({ profile, handleShare, handleCVView, reviews, setIsReviewModalOpen, setIsAppointmentOpen, isAppointmentOpen, t, trackEvent, toneStyle, setIsLeadModalOpen, isLeadModalOpen, setIsQrOpen, setLang, lang, setIsWalletModalOpen, isWalletModalOpen, aiConfig, setIsAIChatOpen, translateText, cvViewUrl, setIsCVModalOpen, isCVModalOpen }: any) {
    const theme = {
        bg: "bg-[#fcfdfd]",
        card: "bg-white",
    ...
    return (
        <div className={cn("min-h-screen relative overflow-x-hidden selection:bg-slate-200", theme.bg, "font-serif")}>
            <header className="sticky top-0 z-[100] px-8 py-6 flex justify-between items-center bg-white/50 backdrop-blur-xl border-b border-slate-50">
                <button onClick={() => setIsQrOpen(true)} className="text-slate-400 hover:text-slate-900 transition-colors">
                    <QrCode size={20} strokeWidth={1.5} />
                </button>
                <div className="flex gap-6 items-center">
                    <button onClick={() => setLang(lang === 'tr' ? 'en' : 'tr')} className="text-[10px] font-bold tracking-[0.2em] text-slate-400 hover:text-slate-900 border-b border-transparent hover:border-slate-900 transition-all">
                        {lang.toUpperCase()}
                    </button>
                    <button onClick={() => setIsWalletModalOpen(true)} className="text-slate-400 hover:text-slate-900 transition-colors">
                        <UserPlus size={20} strokeWidth={1.5} />
                    </button>
                </div>
            </header>

            <main className="relative z-10 w-full max-w-lg mx-auto px-8 py-20 pb-48">
                {/* Identity Area */}
                <section className="flex flex-col items-center text-center space-y-10">
                    <div className="relative">
                        <div className="w-40 h-40 rounded-full p-1 bg-white shadow-[0_20px_50px_rgba(0,0,0,0.05)] border border-slate-100">
                            <img 
                                src={profile?.user?.image || `https://ui-avatars.com/api/?name=${profile?.user?.name || "User"}`} 
                                className="w-full h-full object-cover rounded-full" 
                                alt={profile?.user?.name || ""} 
                    ...
                    <div className="space-y-4">
                        ...
                        <p className="text-[12px] font-medium uppercase tracking-[0.4em] text-slate-400">
                            {translateText(profile.occupation)}
                        </p>
                        {profile.slogan && (
                            <div className="pt-4 max-w-xs mx-auto">
                                <p className="text-sm italic text-slate-500 leading-relaxed border-l-2 border-slate-100 pl-4 text-left">
                                    “{translateText(profile.slogan)}”
                                </p>
                            </div>
                        )}
                ...
                <div className="mt-20 space-y-6">
                    {socialLinks.filter((l: any) => l.platform === 'phone' || l.platform === 'whatsapp' || l.platform === 'website').map((link: any, i: number) => (
                        <motion.a
                            key={i}
                            href={link.platform === 'phone' ? `tel:${link.url}` : link.platform === 'whatsapp' ? `https://wa.me/${link.url.replace(/\D/g, '')}` : link.url}
                            className="flex items-center justify-between py-6 border-b border-slate-100 group transition-all"
                        >
                            <div className="flex flex-col">
                                <span className="text-[9px] font-bold text-slate-300 uppercase tracking-widest mb-1">{link.platform}</span>
                                <span className="text-sm font-medium text-slate-600 group-hover:text-slate-900 transition-colors">{link.platform === 'phone' ? t.phoneCallsBtn || "ARA" : link.platform === 'whatsapp' ? "WHATSAPP" : "WEB SİTE"}</span>
                            </div>
                    ...
                    <motion.button
                        onClick={() => setIsLeadModalOpen(true)}
                        className="w-full py-6 mt-4 flex items-center justify-center gap-3 text-[11px] font-bold uppercase tracking-[0.3em] text-white bg-slate-900 hover:bg-black transition-all shadow-xl"
                    >
                        <Mail size={16} strokeWidth={1.5} />
                        {t.contactMeTitle || "Mesaj Bırakın"}
                    </motion.button>
                </div>
                ...
                <div className="mt-24 space-y-24">
                     {profile.bio && (
                        <div className="space-y-6">
                             <h3 className="text-[10px] font-bold uppercase tracking-[0.4em] text-slate-300">HAKKINDA</h3>
                             <p className="text-sm text-slate-500 leading-relaxed italic antialiased">
                                {translateText(profile.bio)}
                             </p>
                        </div>
                     )}
                    ...
                    {profile.services?.length > 0 && (
                        <div className="space-y-10">
                            <h3 className="text-[10px] font-bold uppercase tracking-[0.4em] text-slate-300">YETKİNLİKLER</h3>
                            <div className="grid grid-cols-1 gap-12">
                                {profile.services.map((service: any, i: number) => (
                                    <div key={i} className="space-y-3">
                                        <h4 className="text-md font-medium text-slate-900 italic">{translateText(service.title)}</h4>
                                        <p className="text-xs text-slate-400 font-sans leading-relaxed">{translateText(service.description)}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
                ...
                {/* Footer Logo */}
                <div className="py-24 flex flex-col items-center opacity-20 hover:opacity-100 transition-opacity">
                     <Link href="/" className="flex items-center gap-2">
                        <span className="text-xs font-medium tracking-tighter text-slate-900 italic">Kardly<span className="underline decoration-slate-400 underline-offset-4">.site</span></span>
                    </Link>
                </div>
            </main>

            {/* Sticky Bottom (Minimalist) */}
            <div className="fixed bottom-10 left-1/2 -translate-x-1/2 w-full max-w-[320px] px-6 z-[150]">
                <div className="bg-white border border-slate-100 shadow-[0_20px_50px_rgba(0,0,0,0.08)] flex items-center divide-x divide-slate-50 overflow-hidden">
                    <button onClick={handleShare} className="flex-1 py-4 text-slate-400 hover:text-slate-900 transition-colors flex justify-center">
                        <Share2 size={18} strokeWidth={1.5} />
                    </button>
                    <button onClick={handleCVView} className="flex-[2] py-4 text-[10px] font-bold tracking-[0.2em] text-slate-900 hover:bg-slate-50 transition-colors">
                        {profile.isCatalog ? "KATALOG" : "CV ÖNİZLE"}
                    </button>
                    <button onClick={() => setIsAppointmentOpen(true)} className="flex-1 py-4 text-slate-400 hover:text-slate-900 transition-colors flex justify-center">
...
function CreativePremiumTemplate({ profile, handleShare, handleCVView, reviews, setIsReviewModalOpen, setIsAppointmentOpen, isAppointmentOpen, t, trackEvent, toneStyle, setIsLeadModalOpen, isLeadModalOpen, setIsQrOpen, setLang, lang, setIsWalletModalOpen, isWalletModalOpen, aiConfig, setIsAIChatOpen, translateText, cvViewUrl, setIsCVModalOpen, isCVModalOpen }: any) {
    const theme = {
        bg: "bg-[#f8f5f0]",
        card: "bg-white/40",
    ...
    return (
        <div className={cn("min-h-screen relative overflow-x-hidden selection:bg-indigo-100", theme.bg, toneStyle.font)}>
            {/* Organic Background Blobs */}
            <div className="fixed inset-0 z-0 pointer-events-none">
                <div className="absolute top-[5%] right-[-10%] w-[60%] h-[60%] bg-indigo-50 blur-[100px] rounded-full opacity-60" />
                <div className="absolute bottom-[10%] left-[-10%] w-[50%] h-[50%] bg-rose-50 blur-[100px] rounded-full opacity-60" />
            </div>

            <header className="sticky top-0 z-[100] px-6 py-6 flex justify-between items-center backdrop-blur-md">
                <button onClick={() => setIsQrOpen(true)} className="w-10 h-10 rounded-full bg-white shadow-sm flex items-center justify-center text-slate-400 hover:text-indigo-600 transition-all border border-white/50">
                    <QrCode size={18} />
                </button>
                <div className="flex gap-4 items-center">
                    <button onClick={() => setLang(lang === 'tr' ? 'en' : 'tr')} className="px-3 py-1 bg-white/50 rounded-full border border-white/30 text-[9px] font-black uppercase tracking-widest text-slate-400 hover:text-slate-900 transition-all">
                        {lang.toUpperCase()}
                    </button>
                    <button onClick={() => setIsWalletModalOpen(true)} className="w-10 h-10 rounded-full bg-slate-900 shadow-lg flex items-center justify-center text-white hover:bg-black transition-all">
                        <UserPlus size={18} />
                    </button>
                </div>
            </header>

            <main className="relative z-10 w-full max-w-lg mx-auto px-6 py-12 pb-48">
                {/* Creative Identity Block */}
                <div className="relative">
                    <div className="absolute -top-10 -left-6 w-32 h-32 bg-indigo-600/5 rounded-full blur-2xl" />
                    <div className="grid grid-cols-12 gap-6 items-center">
                        <div className="col-span-12 flex flex-col items-center text-center space-y-6">
                            <div className="relative">
                                <div className="w-44 h-44 rounded-[3.5rem] bg-white p-2 shadow-[0_40px_80px_-20px_rgba(0,0,0,0.1)] border border-white/80 rotate-3 transition-transform hover:rotate-0">
                                    <img 
                                        src={profile?.user?.image || `https://ui-avatars.com/api/?name=${profile?.user?.name || "User"}`} 
                                ...
                                <div className="absolute -bottom-4 -right-4 w-12 h-12 bg-indigo-600 rounded-2xl flex items-center justify-center text-white shadow-xl shadow-indigo-200">
                                    <Sparkles size={18} />
                                </div>
                            </div>
                            
                            <div className="space-y-3 pt-6">
                                <h1 className="text-5xl font-black italic tracking-tighter text-slate-900 leading-[0.9] uppercase">
                                    {profile?.user?.name || ""}
                                </h1>
                                <p className="text-[12px] font-black uppercase tracking-[0.5em] text-indigo-600/60">
                                    {translateText(profile.occupation)}
                                </p>
                            </div>
                        </div>
                ...
                <div className="mt-16 grid grid-cols-2 gap-4">
                    {socialLinks.filter((l: any) => l.platform === 'phone' || l.platform === 'whatsapp').map((link: any, i: number) => (
                        <motion.a
                            key={i}
                            href={link.platform === 'phone' ? `tel:${link.url}` : `https://wa.me/${link.url.replace(/\D/g, '')}`}
                            whileHover={{ y: -5 }}
                            className="bg-white/40 backdrop-blur-3xl border border-white/60 p-6 rounded-[2.5rem] shadow-sm hover:shadow-xl transition-all flex flex-col gap-4 group"
                        >
                            <div className={cn("w-10 h-10 rounded-2xl flex items-center justify-center transition-all group-hover:bg-indigo-600 group-hover:text-white", link.platform === 'whatsapp' ? "bg-emerald-50 text-emerald-600" : "bg-indigo-50 text-indigo-600")}>
                                {link.platform === 'phone' ? <Phone size={18} /> : <MessageCircle size={18} />}
                            </div>
                            <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 group-hover:text-slate-900">{link.platform === 'phone' ? t.phoneCallsBtn || "HEMEN ARA" : "WHATSAPP"}</span>
                        </motion.a>
                    ))}
                </div>
                ...
                <motion.button
                    onClick={() => setIsLeadModalOpen(true)}
                    className="w-full mt-4 py-8 rounded-[3rem] bg-slate-900 text-white flex items-center justify-center gap-4 group hover:bg-black transition-all shadow-[0_30px_60px_-15px_rgba(0,0,0,0.3)]"
                >
                    ...
                    <span className="text-[12px] font-black uppercase tracking-[0.3em]">{t.contactMeTitle || "PROJE BAŞLAT"}</span>
                    ...
                    {profile.services?.length > 0 && (
                        <div className="space-y-10">
                            <div className="flex items-center gap-3">
                                ...
                                <h3 className="text-[11px] font-black uppercase tracking-[0.4em] text-slate-400 italic">YETKİNLİKLERİM</h3>
                            </div>
                            <div className="grid grid-cols-1 gap-6">
                                {profile.services.map((service: any, i: number) => (
                                    <div key={i} className="flex gap-6 items-start">
                                        <div className="text-[11px] font-black text-indigo-200 mt-1 italic">0{i+1}</div>
                                        <div className="space-y-2">
                                            <h4 className="text-sm font-black text-slate-800 uppercase tracking-widest">{translateText(service.title)}</h4>
                                            <p className="text-[11px] text-slate-400 leading-relaxed font-medium antialiased">{translateText(service.description)}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                ...
                {/* Footer Logo */}
                <div className="py-24 opacity-20 hover:opacity-100 transition-opacity flex justify-center">
                    <Link href="/" className="flex items-center gap-1">
                        <span className="text-[10px] font-black tracking-widest text-slate-900 uppercase">Kardly · Digital Space</span>
                    </Link>
                </div>
            </main>

            {/* Premium Floating Bar */}
            <div className="fixed bottom-10 left-1/2 -translate-x-1/2 w-full max-w-[380px] px-6 z-[150]">
                <div className="bg-white/80 backdrop-blur-2xl border border-white/50 rounded-[3rem] p-2.5 shadow-[0_30px_60px_-12px_rgba(0,0,0,0.1)] flex items-center gap-2">
                     <button onClick={handleShare} className="w-[60px] h-[60px] rounded-[2rem] bg-slate-50 border border-slate-100 flex items-center justify-center text-slate-400 hover:text-indigo-600 transition-all">
                        <Share2 size={18} />
                    </button>
                    <button
                        onClick={handleCVView}
                        className="flex-1 h-[60px] rounded-[2rem] bg-indigo-600 text-white text-[11px] font-black uppercase tracking-[0.3em] shadow-lg shadow-indigo-200 active:scale-95 transition-all"
                    >
                        {profile.isCatalog ? "KATALOG" : "CV ÖNİZLE"}
                    </button>
                    <button onClick={() => setIsAppointmentOpen(true)} className="w-[60px] h-[60px] rounded-[2rem] bg-slate-900 text-amber-300 flex items-center justify-center shadow-lg active:scale-95 transition-all">
                        <Zap size={20} fill="#fcd34d" />
                    </button>
                </div>
            </div>
        </div>
...
function CyberEliteTemplate({ profile, handleShare, handleCVView, reviews, setIsReviewModalOpen, setIsAppointmentOpen, isAppointmentOpen, t, trackEvent, toneStyle, setIsLeadModalOpen, isLeadModalOpen, setIsQrOpen, setLang, lang, setIsWalletModalOpen, isWalletModalOpen, aiConfig, setIsAIChatOpen, translateText, cvViewUrl, setIsCVModalOpen, isCVModalOpen }: any) {
    const theme = {
        bg: "bg-[#0a0a0c]",
        card: "bg-black/60",
    ...
    return (
        <div className={cn("min-h-screen relative overflow-x-hidden selection:bg-yellow-500/30", theme.bg, toneStyle.font)}>
            {/* Cyber Grid Background */}
            <div className="fixed inset-0 z-0 pointer-events-none overflow-hidden">
                <div className="absolute inset-0 bg-[url('/images/patterns/grid.svg')] opacity-[0.03]" />
                <div className="absolute top-0 w-full h-px bg-gradient-to-r from-transparent via-yellow-500/20 to-transparent shadow-[0_0_20px_rgba(250,204,21,0.5)]" />
                <div className="absolute top-1/4 -right-20 w-80 h-80 bg-yellow-500/5 blur-[100px] rounded-full" />
                <div className="absolute bottom-1/4 -left-20 w-80 h-80 bg-blue-500/5 blur-[100px] rounded-full" />
            </div>

            <header className="sticky top-0 z-[100] px-6 py-6 flex justify-between items-center backdrop-blur-2xl">
                <button onClick={() => setIsQrOpen(true)} className="w-10 h-10 rounded-lg bg-black border border-white/10 flex items-center justify-center text-yellow-500 hover:border-yellow-500/50 transition-all shadow-xl">
                    <QrCode size={18} />
                </button>
                <div className="flex gap-4 items-center">
                    <button onClick={() => setLang(lang === 'tr' ? 'en' : 'tr')} className="px-3 py-1 bg-black rounded-lg border border-white/10 text-[9px] font-black uppercase tracking-widest text-slate-500 hover:text-white transition-all">
                        {lang.toUpperCase()}
                    </button>
                    <button onClick={() => setIsWalletModalOpen(true)} className="w-10 h-10 rounded-lg bg-yellow-500 flex items-center justify-center text-black hover:brightness-110 transition-all">
                        <UserPlus size={18} strokeWidth={2.5} />
                    </button>
                </div>
            </header>

            <main className="relative z-10 w-full max-w-lg mx-auto px-6 py-12 pb-48 space-y-16">
                {/* Tech Identity */}
                <section className="flex flex-col items-center text-center space-y-8">
                    <div className="relative group">
                        <div className="absolute -inset-1 bg-gradient-to-tr from-yellow-500 to-amber-600 rounded-2xl blur opacity-25 group-hover:opacity-50 transition duration-1000"></div>
                        <div className="relative w-40 h-40 rounded-2xl bg-black border border-white/10 p-2 overflow-hidden shadow-2xl">
                            <img 
                                src={profile?.user?.image || `https://ui-avatars.com/api/?name=${profile?.user?.name || "User"}`} 
                                className="w-full h-full object-cover rounded-xl filter contrast-[1.1]" 
                                alt={profile?.user?.name || ""} 
                            />
                            {/* Scanning Line Animation */}
                            <div className="absolute top-0 left-0 w-full h-px bg-yellow-500/50 shadow-[0_0_10px_#facc15] animate-scan" />
                        </div>
                    </div>

                    <div className="space-y-4">
                        <h1 className="text-4xl font-black tracking-tight text-white uppercase drop-shadow-[0_0_10px_rgba(255,255,255,0.3)]">
                            {profile?.user?.name || ""}
                        </h1>
                        <div className="w-fit mx-auto px-6 py-2 bg-yellow-500/10 border border-yellow-500/20 rounded-md">
                            <p className="text-[11px] font-black uppercase tracking-[0.4em] text-yellow-500">
                                {translateText(profile.occupation)}
                            </p>
                        </div>
                    </div>
                ...
                <div className="grid grid-cols-1 gap-4">
                    {socialLinks.filter((l: any) => l.platform === 'phone' || l.platform === 'whatsapp' || l.platform === 'website').map((link: any, i: number) => (
                        <motion.a
                            ...
                            href={link.platform === 'phone' ? `tel:${link.url}` : link.platform === 'whatsapp' ? `https://wa.me/${link.url.replace(/\D/g, '')}` : link.url}
                            whileHover={{ scale: 1.02 }}
                            className="bg-black/40 border border-white/5 p-6 rounded-xl flex items-center justify-between group transition-all hover:border-yellow-500/30"
                        >
                            <div className="flex items-center gap-6">
                                <div className="w-12 h-12 bg-white/5 rounded-lg flex items-center justify-center text-white/40 group-hover:text-yellow-500 transition-colors">
                                    {link.platform === 'phone' && <Phone size={20} />}
                                    {link.platform === 'whatsapp' && <MessageCircle size={20} />}
                                    {link.platform === 'website' && <Globe size={20} />}
                                </div>
                                <div className="flex flex-col">
                                    <span className="text-[8px] font-black text-white/20 uppercase tracking-[0.3em] mb-1">{link.platform}</span>
                                    <span className="text-xs font-black text-white/70 uppercase tracking-widest">{link.platform === 'phone' ? t.phoneCallsBtn || "ARA" : "İLETİŞİM"}</span>
                                </div>
                            </div>
                            <div className="text-white/10 group-hover:text-yellow-500/40">
                                 <ArrowRight size={20} />
                            </div>
                        </motion.a>
                    ))}
                    ...
                    <motion.button
                        onClick={() => setIsLeadModalOpen(true)}
                        className="w-full h-20 bg-yellow-500 text-black flex items-center justify-center gap-4 group transition-all hover:bg-white shadow-[0_0_30px_rgba(250,204,21,0.2)]"
                    >
                        <Cpu size={20} strokeWidth={2.5} />
                        <span className="text-[12px] font-black uppercase tracking-[0.4em]">{t.contactMeTitle || "PROTOKOL BAŞLAT"}</span>
                    </motion.button>
                </div>
                    ...
                    {profile.services?.length > 0 && (
                        <div className="space-y-8">
                             <div className="flex items-center gap-4">
                                ...
                                <h3 className="text-[10px] font-black uppercase tracking-[0.4em] text-white/30 italic">SİSTEM YETKİNLİKLERİ</h3>
                             </div>
                             <div className="grid grid-cols-1 gap-6">
                                {profile.services.map((item: any, i: number) => (
                                    <div key={i} className="p-8 bg-white/[0.02] border border-white/5 rounded-2xl group hover:border-yellow-500/20 transition-all">
                                        <div className="flex justify-between items-start mb-4">
                                            <h4 className="text-sm font-black text-white uppercase tracking-widest">{translateText(item.title)}</h4>
                                            <span className="text-[10px] font-black text-yellow-500/20">CORE.LOG</span>
                                        </div>
                                        <p className="text-[11px] text-white/40 leading-relaxed font-medium">{translateText(item.description)}</p>
                                    </div>
                                ))}
                ...
                {/* Footer Logo */}
                <div className="py-24 flex flex-col items-center gap-6 opacity-40 grayscale transition-all hover:grayscale-0 hover:opacity-100">
                    <div className="h-px w-20 bg-yellow-500/20" />
                     <Link href="/" className="flex items-center gap-2">
                        <span className="text-[10px] font-black tracking-[0.5em] text-white uppercase">KARDLY_SYS</span>
                    </Link>
                </div>
            </main>

            {/* Tech Bottom Action Bar */}
            <div className="fixed bottom-10 left-1/2 -translate-x-1/2 w-full max-w-[440px] px-6 z-[150]">
                <div className="bg-black border border-white/10 p-3 shadow-[0_0_50px_rgba(0,0,0,0.8)] flex items-center justify-between gap-3">
                    <button onClick={handleShare} className="w-14 h-14 bg-white/5 border border-white/10 flex items-center justify-center text-white/40 hover:text-yellow-500 transition-all">
                        <Share2 size={20} />
                    </button>

                    <button
                        onClick={handleCVView}
                        className="flex-1 h-14 bg-black border-2 border-yellow-500 text-yellow-500 text-[11px] font-black uppercase tracking-[0.3em] hover:bg-yellow-500 hover:text-black transition-all group"
                    >
                        <span className="group-hover:px-2 transition-all">{profile.isCatalog ? "ACCESS_CATALOG" : "DOWNLOAD_CV"}</span>
                    </button>

                    <button onClick={() => setIsAppointmentOpen(true)} className="w-14 h-14 bg-yellow-500 text-black flex items-center justify-center hover:bg-white transition-all">
                        <Calendar size={20} />
                    </button>
                    
                    {aiConfig?.isEnabled && (
                        <button onClick={() => setIsAIChatOpen(true)} className="w-14 h-14 bg-white/5 border border-white/10 flex items-center justify-center text-yellow-500 transition-all hover:bg-yellow-500 hover:text-black group">
                            <Bot size={22} className="group-hover:animate-spin" />
                        </button>
                    )}
                </div>
            </div>
